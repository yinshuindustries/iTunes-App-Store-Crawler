...
def site_open(site):
    ...
    try:
        ...
        req.add_header("User-Agent","Mozilla/5.0 (Macintosh; Intel Mac OS X 10_6_8) AppleWebKit/536.5 (KHTML, like Gecko) Chrome/19.0.1084.46 Safari/536.5")
...
def category_get(soup):
    ...
    return soup.find(class_="information-list information-list--app medium-columns").find(class_="link").text
...
def size_get(soup):
    ...
    return soup.find(class_="information-list information-list--app medium-columns").find(attrs={"aria-label":True}).text
...
def rating_get(soup):
    ...
    stars = soup.find(class_="we-customer-ratings__averages__display").text
    rating = soup.find(class_="we-customer-ratings__count small-hide medium-show").text

    #return a tuple of stars and rating without whitespace
    return (stars.strip(),rating.strip())

def id_get(soup):
    return soup.find(attrs={"name":"apple:content_id"})['content']

def url_get(soup):
    return soup.find(attrs={"rel":"canonical"})['href']

def description_get(soup):
    return soup.find(class_="we-truncate we-truncate--multi-line we-truncate--interactive ember-view")['aria-label']

def age_get(soup):
    return soup.find(class_="badge badge--product-title").text

def version_get(soup):
    return soup.find(class_="version-history__item__version-number").text

def release_date_get(soup):
    return soup.find(class_="version-history__item__release-date").text
...
def app_info(soup,site):
    ...
    try:
        ...
        app_url = url_get(soup)
        description = description_get(soup)
        age = age_get(soup)
        version = version_get(soup)
        release_date = release_date_get(soup)

        #returns tuple of the app information
        return (app_id, app_url, title, developer, stars, rating, description, version, release_date, size, category, age, price)
    except:
...
def app_crawl_main_loop(data,writer):
    ...
    for i,link in enumerate(data):
        ...
        try:
            info = app_info(soup_site(link.strip('\n|"')),link.strip('\n|"'))
            #opens the site, parses out the data, and writes it into the csv
            writer.writerow(info)
            print('=================================', info[0], 'scraped successfully ===================================')
        except:
